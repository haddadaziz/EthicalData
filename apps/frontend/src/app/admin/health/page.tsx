    ...
    if (loading) {
        return (
            <div className="p-16 text-center text-slate-400 bg-[#080d1a] border border-slate-800 rounded-3xl max-w-5xl mx-auto">
                ...
                <p className="text-xs font-bold uppercase tracking-widest text-cyan-400">Connexion au sondeur de santé...</p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-8 max-w-6xl mx-auto text-left bg-[#020617]">
            ...
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 bg-[#080d1a] p-6 md:p-8 rounded-3xl border border-slate-800 shadow-sm">
                ...
                <div className="flex items-center gap-2 px-4 py-2 bg-emerald-950/30 border border-emerald-800/50 rounded-2xl text-emerald-400 text-xs font-bold">
            ...
            {health && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
                    {/* STATUT SERVEUR */}
                    <div className="p-6 bg-[#080d1a] border border-slate-800 rounded-3xl space-y-3 shadow-sm">
                        <div className="flex items-center justify-between">
                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Statut API</span>
                            <div className={`w-9 h-9 rounded-2xl flex items-center justify-center ${health.status === 'ok' ? 'bg-emerald-950/30 text-emerald-400' : 'bg-rose-950/30 text-rose-400'}`}>
                                <CheckCircle className="w-4 h-4" />
                            </div>
                        </div>
                        <div>
                            <span className="text-2xl font-black text-white uppercase">{health.status}</span>
                            <p className="text-[11px] text-emerald-400 font-bold mt-1">Serveur 100% Opérationnel</p>
                        </div>
                    </div>
                    ...
                    <div className="p-6 bg-[#080d1a] border border-slate-800 rounded-3xl space-y-3 shadow-sm">
                        ...
                        <div>
                            <span className="text-2xl font-black text-white capitalize">{health.database}</span>
                            <p className="text-[11px] text-cyan-400 font-bold mt-1">Connexion active (Prisma)</p>
                        </div>
                    </div>
                    ...
                    <div className="p-6 bg-[#080d1a] border border-slate-800 rounded-3xl space-y-3 shadow-sm">
                        ...
                        <div>
                            <span className="text-2xl font-black text-white">{formatUptime(health.uptimeSeconds)}</span>
                            <p className="text-[11px] text-slate-400 font-bold mt-1">Temps sans interruption</p>
                        </div>
                    </div>
                    ...
                    <div className="p-6 bg-[#080d1a] border border-slate-800 rounded-3xl space-y-3 shadow-sm">
                        ...
                        <div>
                            <span className="text-2xl font-black text-white">{latency} ms</span>
                            <p className="text-[11px] text-purple-400 font-bold mt-1">Temps de réponse de l'API</p>
                        </div>
                    </div>
                </div>
            )}
            ...
            {health && (
                <div className="bg-[#080d1a] border border-slate-800 rounded-3xl p-6 md:p-8 space-y-6 shadow-sm">
                    <h3 className="text-base font-black text-white">Consommation Mémoire RAM (Node.js)</h3>

                    <div className="space-y-4">
                        <div className="space-y-2">
                            <div className="flex items-center justify-between text-xs font-bold">
                                <span className="text-slate-400">Mémoire Utilisée (Heap Used)</span>
                                <span className="text-white">{health.memoryUsageMB.heapUsed} Mo / {health.memoryUsageMB.heapTotal} Mo</span>
                            </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-4 border-t border-slate-800 text-xs">
                            <div className="p-4 bg-[#020617] rounded-2xl space-y-1">
                                <span className="text-[10px] font-black text-slate-400 uppercase">Resident Set Size (RSS)</span>
                                <p className="text-sm font-black text-white">{health.memoryUsageMB.rss} Mo</p>
                            </div>
                            <div className="p-4 bg-[#020617] rounded-2xl space-y-1">
                                <span className="text-[10px] font-black text-slate-400 uppercase">Horodatage Serveur</span>
                                <p className="text-sm font-black text-white">{new Date(health.timestamp).toLocaleTimeString('fr-FR')}</p>
                            </div>
                            <div className="p-4 bg-[#020617] rounded-2xl space-y-1">
                                <span className="text-[10px] font-black text-slate-400 uppercase">Moteur de Recherche BDD</span>
                                <p className="text-sm font-black text-emerald-400">PostgreSQL 16 (Connected)</p>
                            </div>
                        </div>
                    </div>
                </div>
