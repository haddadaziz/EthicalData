...
export default function CoursesPage() {
    ...
    const handlePublish = async (coursId: string) => {
        // Vérification préalable côté frontend
        ...
        try {
            ...
            // Le backend renvoie un tableau d'erreurs → on les affiche toutes
    ...
    const handleDeleteModule = async (moduleId: string) => {
        const ok = await confirm({
            ...
            message: "Cette action supprimera le module et toutes ses ressources.",
    ...
    // ── Création ou édition en cours → afficher l'éditeur ──
        ...
        return (
            <CourseEditor
                ...
                onSave={async (data) => {
                    try {
                        if (editingCours) {
                            await apiFetch(`/cours/${editingCours.id}`, {
                                method: 'PATCH',
                                body: data,
                            });
                    ...
                    {filteredCours.map((c) => (
                        <div key={c.id} className="group bg-white border border-slate-200/80 rounded-2xl overflow-hidden shadow-xs hover:shadow-lg hover:border-slate-300 transition-all duration-300 flex flex-col">
                            ...
                            <div className="border-t border-slate-100 p-3 flex items-center gap-2">
                                <button onClick={() => setEditingCours(c)}
                                ...
                                <button onClick={() => handleDeleteCours(c.id)}
