...
export default function LearnerCoursesPage() {
    ...
    return (
        <div className="space-y-8 text-white text-left max-w-7xl mx-auto">
                ...
                <AnimatePresence mode="wait">
                    {filteredDisponibles.length === 0 ? (
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                            className="p-12 text-center bg-[#080d1a] border border-slate-800 rounded-3xl text-slate-400 font-semibold">
                                ...
                                <div key={c.id} className="group bg-[#080d1a] border border-blue-900/50 shadow-[0_0_25px_rgba(30,58,138,0.05)] rounded-2xl overflow-hidden hover:shadow-[0_0_35px_rgba(30,58,138,0.2)] hover:border-blue-700/60 transition-all duration-300 flex flex-col">
                                    ...
                                    <div className="p-4 flex-1 flex flex-col gap-2">
                                        <div className="flex items-center gap-1.5">
                                            <span className="text-[9px] font-black text-slate-300 bg-slate-800 px-2 py-0.5 rounded uppercase tracking-wider border border-slate-700">
                                                {c.certification?.fournisseur?.nom || 'Général'}
                                            </span>
                                            <span className="text-[9px] font-black text-emerald-400 bg-emerald-950/30 px-2 py-0.5 rounded uppercase tracking-wider border border-emerald-900/40">
                                ...
                                return (
                                    <div key={insc.id} className="group bg-[#080d1a] border border-blue-900/50 shadow-[0_0_25px_rgba(30,58,138,0.05)] rounded-2xl overflow-hidden hover:shadow-[0_0_35px_rgba(30,58,138,0.2)] hover:border-blue-700/60 transition-all duration-300 flex flex-col">
                                        ...
                                        <div className="mt-auto pt-2 space-y-2">
                                            ...
                                            <div className="w-full h-2 bg-[#020617] border border-slate-800 rounded-full overflow-hidden">
                                    ...
                                    <div className="border-t border-slate-800 p-3">
                                            <button onClick={() => router.push(`/dashboard/cours/${c.id}/apprendre?from=mes-cours`)}
                                                className="w-full px-3 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-[11px] font-black rounded-xl transition-all cursor-pointer flex items-center justify-center gap-1.5 shadow-[0_0_15px_rgba(37,99,235,0.15)]">
                                ...
                                return (
                                    <div key={insc.id} className="group bg-[#080d1a] border border-emerald-900/50 shadow-[0_0_25px_rgba(16,185,129,0.05)] rounded-2xl overflow-hidden hover:shadow-[0_0_35px_rgba(16,185,129,0.2)] hover:border-emerald-700/60 transition-all duration-300 flex flex-col">
                                        <div className="relative aspect-[750/422] bg-[#020617] overflow-hidden">
                                            ...
                                            <span className="absolute top-3 right-3 px-2 py-0.5 bg-emerald-950/80 border border-emerald-900/50 text-emerald-500 text-[8px] font-extrabold rounded-full uppercase tracking-wider flex items-center gap-1 backdrop-blur-sm">
                                                <CheckCircle className="w-3 h-3" /> Terminé
                                            </span>
                                        </div>
                                        <div className="p-4 flex-1 flex flex-col gap-2">
                                            <h3 className="text-sm font-black text-white leading-snug line-clamp-2 group-hover:text-emerald-500 transition-colors">
                                                {c.titre}
                                            </h3>
                                            <p className="text-[11px] text-slate-400 font-medium line-clamp-2 leading-relaxed">
                                                {c.description}
                                            ...
                                            <div className="flex items-center gap-1.5 text-[10px] text-slate-500 font-semibold">
                                                <div className="w-5 h-5 rounded-full bg-slate-800 text-slate-400 border border-slate-700 flex items-center justify-center text-[8px] font-black shrink-0">
                                                    {c.formateur?.avatar ? (
                                                        <img src={c.formateur.avatar} alt="" className="w-full h-full object-cover rounded-full" />
                                                    ) : (
                                                        c.formateur?.prenom?.[0] || 'F'
                                        ...
                                        <div className="border-t border-emerald-900/20 p-3">
                                            <button onClick={() => router.push(`/dashboard/cours/${c.id}/apprendre?from=termines`)}
                                                className="w-full px-3 py-2.5 bg-emerald-950/30 hover:bg-emerald-900/40 text-emerald-500 border border-emerald-900/50 text-[11px] font-black rounded-xl transition-all cursor-pointer flex items-center justify-center gap-1.5">
