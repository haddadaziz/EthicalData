            ...
            >
              ...
              <div className="absolute inset-0 z-0 overflow-hidden">
                <img
                  src={item.bgImage}
                  alt={item.title}
                  className="w-full h-full object-cover opacity-40 group-hover:opacity-60 group-hover:scale-110 transition-all duration-700 ease-out transform-gpu"
                  loading="lazy"
                  decoding="async"
                />
                {/* Gradient sombre pour lisibilité parfaite du texte */}
                <div className={`absolute inset-0 bg-gradient-to-t from-[#020617] via-[#020617]/90 to-[#020617]/40 ${item.accentColor} opacity-95 group-hover:opacity-90 transition-opacity duration-500`} />
              </div>

              {/* Titre & Description au Premier Plan sur fond verrier avec accents cybernétiques */}
              <div className="relative z-10 p-5 md:p-6 space-y-2.5 bg-gradient-to-t from-[#020617] via-[#020617]/95 to-transparent pt-6">
                <h3 className="text-lg font-black text-white group-hover:text-cyan-300 transition-colors duration-300 tracking-tight">
                  {item.title}
                </h3>
