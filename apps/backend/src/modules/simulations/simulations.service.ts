...
export class SimulationsService {
    ...
    return {
      ...
      certification: simulation.certification
        ? {
            ...
            id: simulation.certification.id.toString(),
          }
        : null,
      cours: simulation.cours
        ? { ...simulation.cours, id: simulation.cours.id.toString() }
        : null,
  ...
  async getReadinessScoreForCertification(userId: number, certId: number) {
    ...
    const moduleTitles = (cert.cours || []).flatMap((c) =>
      c.modules.map((m) => m.titre),
    );
    ...
    if (moduleTitles.length >= 2) {
      if (readinessScore >= 80) {
        ...
        if (lacunes.length === 0)
          lacunes = ['Optimisation avancée du temps par question'];
      } else if (readinessScore >= 65) {
      ...
      if (readinessScore >= 80) {
        ...
        lacunes = [
          'Gestion des coûts et calcul de facturation',
          'Sécurité des données et stockage Cloud',
        ];
      } else {
        pointsForts = ["Notions globales d'introduction"];
        lacunes = [
          'Architecture réseau & sécurité',
          'Stockage et redondance de données',
          'Gestion des coûts & facturation',
        ];
      }
    }
  ...
  async createCourseSimulation(coursId: number, dto: CreateSimulationDto) {
    ...
    if (existing)
      throw new BadRequestException('Ce cours a déjà une simulation.');
  ...
  async createCourseTentative(userId: number, coursId: number, score: number) {
    ...
    if (!simulation)
      throw new NotFoundException('Aucune simulation pour ce cours.');
  ...
  async createCourseQuestion(coursId: number, dto: CreateQuestionDto) {
    ...
    if (!simulation)
      throw new NotFoundException(
        "Ce cours n'a pas de simulation. Créez d'abord la simulation.",
      );
  ...
  async getReadinessScoreForCourse(userId: number, coursId: number) {
    ...
    if (!simulation) {
      return {
        ...
        conseil:
          "Le formateur n'a pas encore créé de simulation pour ce cours.",
        pointsForts: [],
        lacunes: [],
        planRevision: [],
        history: [],
    ...
    if (tentatives.length === 0) {
      return {
        ...
        totalTentatives: 0,
        conseil:
          'Veuillez passer la simulation de ce cours pour obtenir une analyse de votre niveau.',
        pointsForts: [],
        lacunes: [],
        planRevision: [
